...
def get_dataloader(batch_size=128, num_workers=2, return_idx=(False, False, False),
    ...
    # preprocessing
    # Pytorch default backend for images are Pillow, 
    # and when you use ToTensor() class, PyTorch automatically converts all images into [0,1].
    data_transform = transforms.Compose([transforms.ToTensor()])
    #transforms.Normalize(mean=[.5], std=[.5]) is discarded here for adding noise
    # standarization is added to the top of the model
...
if __name__ == "__main__":
    # just for testing
    train, val, test = get_dataloader(batch_size=128, num_workers=2, return_idx=(True, False, False),
                   shuffle=(True, False, False), image_size=None,
                   path=r"C:\Research\IMA_v1\IMAv1\data\MedMNIST")
    temp = next(iter(val))
    print(temp[0].shape)
    print("done")