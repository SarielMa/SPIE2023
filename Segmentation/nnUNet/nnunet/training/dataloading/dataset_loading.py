...
class DataLoader2D(SlimDataLoaderBase):
    # add id for each sample so that IMA can be applied
...
class MyTrainDataLoader2D(SlimDataLoaderBase):
    # only use slices with full classes as training sample, not used in this paper
    def __init__(self, data, patch_size, final_patch_size, batch_size, oversample_foreground_percent=0.0,
                 memmap_mode="r", pseudo_3d_slices=1, pad_mode="edge",
                 pad_kwargs_data=None, pad_sides=None):
        """
    ...
    def generate_train_batch(self):
        ...
        keys = selected_keys
        return {'data': data, 'seg': seg, 'properties': case_properties, "keys": keys, "id": ids}
class MyDataLoader2D(SlimDataLoaderBase):
    # use only slices with all classes to evaluate the model, only used in test and validation process.
    def __init__(self, data, patch_size, final_patch_size, batch_size, oversample_foreground_percent=0.0,
                 memmap_mode="r", pseudo_3d_slices=1, pad_mode="edge",
                 pad_kwargs_data=None, pad_sides=None):
        """
        ...
        :param pseudo_3d_slices: 7 = 3 below and 3 above the center slice
        """
        super(MyDataLoader2D, self).__init__(data, batch_size, None)
        if pad_kwargs_data is None:
            pad_kwargs_data = OrderedDict()
        self.pad_kwargs_data = pad_kwargs_data
        self.pad_mode = pad_mode
        ...
        self.data_shape, self.seg_shape = self.determine_shapes()
        #================================================================
        self.counter = 0;# total number of slices
        self.idx = dict()# current_slice-> (key, local_slice_id)
        self.initialize_id()
    ...
    def get_slice(self):
        ...
        if not isfile(self._data[selected_key]['data_file'][:-4] + ".npy"):
            # lets hope you know what you're doing
            case_all_data = np.load(self._data[selected_key]['data_file'][:-4] + ".npz")['data']
        else:
            case_all_data = np.load(self._data[selected_key]['data_file'][:-4] + ".npy", self.memmap_mode) 
                    # this is for when there is just a 2d slice in case_all_data (2d support)
        if len(case_all_data.shape) == 3:
            case_all_data = case_all_data[:, None]   
    ...
    def generate_train_batch(self):
        ...
        for  i in range(self.batch_size):
            ...
            if self.pseudo_3d_slices == 1:
                case_all_data = self.get_slice()
            else:
                raise RuntimeError("self.pseudo_3d_slices != 1!")
            ...
            data[i] = case_all_data_donly
            seg[i] = case_all_data_segonly
            


        return {'data': data, 'seg': seg, 'last': self.lastBatch}

if __name__ == "__main__":
    t = "Task002_Heart"
    p = join(preprocessing_output_dir, t, "nnUNetData_plans_v2.1_2D_stage0")
    dataset = load_dataset(p)
    with open(join(join(preprocessing_output_dir, t), "nnUNetPlansv2.1_plans_2D.pkl"), 'rb') as f:
        plans = pickle.load(f)
    unpack_dataset(p)
    """
