...
from nnunet.ours.RobustDNN_our_claregseg import our_loss
...
from nnunet.ours.PGD import pgd_attack
...
class nnUNetTrainerV2(nnUNetTrainer):
    ...
    def classify_model_std_output_seg(self,Yp, Y):
        ...
        Yp_e_Y=(dice>=0)
        return Yp_e_Y
    # the threshold mentioned in the paper
    ...
    def run_our_iteration(self, data_generator, args,flag1, flag2, E_new, do_backprop=True, run_online_evaluation=False):
        """
        gradient clipping improves training stability
        ...
        data = maybe_to_torch(data)
        target = maybe_to_torch(target)
        # parameters that need to be set for our
        ###################################################
        stop=args.stop
        ...
        loss, loss1, loss2, loss3, Yp, advc, Xn, Ypn, idx_n = our_loss(self.network, data, Y=target,#Y is a tuple
    ...
    def run_our_training(self):
        ...
        ds = self.network.do_ds
        self.network.do_ds = True
        ret = super().run_our_training(self.dl_tr.counter)#pass the number of samples in training set
        self.network.do_ds = ds
        return ret
