...
from nnunet.ours.RobustDNN_our_claregseg import our_update_margin
import os.path
...
class NetworkTrainer(object):
    ...
    def run_our_training(self, counter):
        if not torch.cuda.is_available():
            self.print_to_log_file("WARNING!!! You are attempting to run training on a CPU (torch.cuda.is_available() is False). This can be VERY slow!")
        ...
        while self.epoch < self.max_num_epochs:
            ...
            self.network.train()
            for c in range(self.num_batches_per_epoch):
                l, flag1, flag2, E_new = self.run_our_iteration(self.tr_gen, args,flag1, flag2, E_new, True)
            ...
            self.print_to_log_file("train loss : %.4f" % self.all_tr_losses[-1])
            #------update the margin
            our_update_margin(args, args.delta, args.noise, flag1, flag2, E_new)
            print('our_update_margin: done, margin updated')    
        ...
        # now we can delete latest as it will be identical with final
        if isfile(join(self.output_folder, "model_our_latest.model")):
            os.remove(join(self.output_folder, "model_our_latest.model"))
        if isfile(join(self.output_folder, "model_our_latest.model.pkl")):
            os.remove(join(self.output_folder, "model_our_latest.model.pkl"))
