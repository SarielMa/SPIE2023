...
from nnunet.ours.RobustDNN_PGD import get_noise_init, normalize_grad_, clip_norm_
#%% classification, see more advanced loss functions in RobustDNN_our
...
def our_loss(model, X, Y, margin, norm_type, max_iter, step,
    ...
    if enable_loss3 == True:
        ...
        # to be consistant with the output of our_loss in RobustDNN_our
        Xn=Xn[idx_n] 
        #Ypn=Ypn[idx_n]
        if idx_n.size(0)>0:   
...
#%%
def our_check_margin_one_batch(model, X, Y, margin, norm_type, max_iter, step,
                               rand_init_norm=None,
                               clip_X_min=0, clip_X_max=1,
                               refine_Xn_max_iter=10,
                               Xn1_equal_X=False,
...
def our_check_margin(model, device, dataloader,
    ...
    for batch_idx, (X, Y, Idx) in enumerate(dataloader):
        ...
        flag1_, flag2_, margin_ = our_check_margin_one_batch(model, X, Y, margin_, norm_type, max_iter, step_,
    ...
    return flag1, flag2, margin_new
#%%
def our_update_margin_OLD(margin, delta, max_margin, flag1, flag2, margin_new):
    # margin: to be updated
    # delta: margin expansion step size
    # max_margin: maximum margin
    # flag1, flag2, margin_new: from our_check_margin
...
def our_update_margin(args, delta, max_margin, flag1, flag2, margin_new):
    # margin: to be updated
    # delta: margin expansion step size
    # max_margin: maximum margin
    # flag1, flag2, margin_new: from our_check_margin
...
#%%
def our_estimate_margin(model, device, dataloader,
    ...
    for n in range(1, len(margin_level)):
        ...
        flag1, flag2, Enew =our_check_margin(model, device, dataloader,
